...
from .sign_arbitration import lane_binding_metadata
from .signs import minimum_speed_value, policy_for_label, resolve_sign_message, speed_value
...
class RiskEngine:
    ...
    def analyze(
        ...
        for track in tracks:
            ...
            if (
                ...
                candidates.append(
                    self._candidate(
                        "vulnerable_road_user",
                        "warning",
                        vulnerable_message(label_vi, location),
                        item,
                        evidence,
                    )
                )
            ...
            cut_in_eligible = track["label"] in {
                "car",
                "bus",
                "truck",
                "motorcycle",
            ...
            if (
                ...
                candidates.append(
                    self._candidate(
                        "cut_in",
                        "warning",
                        cut_in_message(label_vi, origin_location),
            ...
            pedestrian_safe_crossing = (
                track["label"] == "person"
                and path_conflict
                and not near_field_imminent
                and risk < float(self.config.get("vulnerable_warning", 0.52))
            )
            vehicle_crossing_relevant = (
                track["label"]
                in {"rider", "motorcycle", "bicycle", "car", "bus", "truck"}
                and (
                    risk >= float(self.config.get("cross_traffic_warning", 0.48))
                    or proximity
                    >= float(self.config.get("cross_traffic_proximity_override", 0.42))
                    or abs(relative_lateral)
                    >= float(self.config.get("cross_traffic_fast_lateral_min", 0.045))
                    or approach >= float(self.config.get("cross_traffic_approach_min", 0.05))
                )
            )
            if (
                track["label"]
                ...
                and (pedestrian_safe_crossing or vehicle_crossing_relevant)
                ...
                candidates.append(
                    self._candidate(
                        "cross_traffic",
                        "warning",
                        cross_traffic_message(label_vi, origin_side, movement_direction),
                        item,
                        {
                            **evidence,
                            ...
                            "crossing_profile": (
                                "pedestrian_safe_crossing"
                                if pedestrian_safe_crossing
                                else "vehicle_conflict_or_fast_crossing"
                            ),
            ...
            for class_id, (sign, policy, visual_score) in best_by_class.items():
                ...
                stable_box = (
                    ...
                    and self._traffic_sign_track_stable(
                        ...
                        sign["bbox"],
                        frame_shape,
                        max(0.0, now - self._sign_last_seen.get(class_id, now)),
                    )
                )
                if not stable_box:
                ...
                if confirmed:
                    maximum_speed = speed_value(str(sign["label"]))
                    minimum_speed = minimum_speed_value(str(sign["label"]))
                    speed = maximum_speed if maximum_speed is not None else minimum_speed
                    speed_role = (
                        "maximum"
                    ...
                    event_type = "speed_sign" if speed_role is not None else "traffic_sign"
                    sign_message, sign_audio_eligible, sign_copy_evidence = resolve_sign_message(
                        str(sign["label"]), sign
                    )
                    lane_binding = lane_binding_metadata(sign)
                    candidates.append(
                        {
                            ...
                            "location": "phía trước",
                            "cooldown_key": (
                                f"speed_sign:{'minimum:' if speed_role == 'minimum' else ''}{speed}"
                                if speed is not None
                                else f"traffic_sign:{class_id}"
                            ),
                            "is_traffic_sign": True,
                            "audio_eligible": sign_audio_eligible,
                            "evidence": {
                                "class_id": class_id,
                                "speed_value": speed,
    ...
    def _traffic_sign_track_stable(
        ...
        base_distance = float(
            self.config.get("traffic_sign_tracking_center_distance_max", 0.055)
        )
        velocity_distance = float(
            self.config.get("traffic_sign_tracking_center_velocity_max_per_second", 0.09)
        ) * max(0.0, float(elapsed_seconds))
        center_distance_limit = min(
            float(self.config.get("traffic_sign_tracking_center_distance_cap", 0.12)),
            max(base_distance, velocity_distance),
        )
        return (
    ...
    def _candidate(
        ...
        return {
            "event_type": event_type,
            "severity": severity,
            "message": validate_alert_message(message),
