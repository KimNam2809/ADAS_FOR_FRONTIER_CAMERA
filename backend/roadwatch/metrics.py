...
class MetricsCollector:
    def __init__(self, window: int = 240) -> None:
        ...
        # Display telemetry is deliberately separate from inference telemetry.
        # A display frame can be a sampled frame or a latest-state rendering;
        # it must never count as a processed/risk-evaluated frame.
        self.display_frames = 0
        self.display_dropped_frames = 0
        self.display_started_at: float | None = None
    ...
    def mark_processing_started(self) -> None:
        """Start the FPS clock after model warmup, not at session creation."""
        with self._lock:
    ...
    def record_display_frame(self) -> None:
        with self._lock:
            if self.display_started_at is None:
                self.display_started_at = time.time()
            self.display_frames += 1

    def record_display_drop(self) -> None:
        with self._lock:
            self.display_dropped_frames += 1
    ...
    def snapshot(self) -> dict[str, Any]:
        with self._lock:
            ...
            processing_elapsed = max(
                time.time() - self.processing_started_at, 0.001
            ) if self.processing_started_at is not None else elapsed
            display_elapsed = max(
                time.time() - self.display_started_at, 0.001
            ) if self.display_started_at is not None else 0.0
            latencies = {}
            ...
            return {
                ...
                # Processed FPS is measured only after warmup and is not tied
                # to browser repaint or MJPEG delivery.
                "processed_fps": round(self.processed_frames / processing_elapsed, 2),
                "display_fps": round(
                    self.display_frames / display_elapsed, 2
                ) if self.display_started_at is not None else 0.0,
                "display_frames": self.display_frames,
                "display_dropped_frames": self.display_dropped_frames,
