...
function VideoStage({ token, status, compact = false }: { token: string; status: Status; compact?: boolean }) {
  const topEvent = status.events[0];
  return <section className={`video-stage ${compact ? "compact" : ""}`}>
    ...
    {topEvent && status.running && <div className={`hazard-banner ${topEvent.severity}`}><span>{topEvent.severity === "critical" ? "!" : "i"}</span><div><small>{topEvent.event_type.toUpperCase()}</small><strong>{topEvent.message}</strong></div><b>{Math.round(topEvent.risk_score * 100)}</b></div>}
  </section>;
}
  ...
  return <main className="page driver-page">
    ...
    <div className="driver-layout">
      ...
      <aside className="driver-rail">
        <div className="focus-card"><span>NGUY CƠ HIỆN TẠI</span><strong>{status.events[0] ? Math.round(status.events[0].risk_score * 100) : 0}<small>/100</small></strong><div className="risk-meter"><i style={{ width: `${status.events[0] ? status.events[0].risk_score * 100 : 0}%` }} /></div><p>{status.events[0]?.message ?? "Không có cảnh báo cần chú ý."}</p></div>
...
function EventRibbon({ events }: { events: AlertEvent[] }) {
  return <section className="event-ribbon"><div className="section-title"><span>SỰ KIỆN GẦN NHẤT</span><small>Evidence timeline</small></div><div className="ribbon-list">{events.length ? events.map((event, index) => <article key={`${event.id ?? index}-${event.created_at}`} className={event.severity}><i /><div><strong>{event.message}</strong><span>{new Date(event.created_at * 1000).toLocaleTimeString("vi-VN")} · risk {Math.round(event.risk_score * 100)} · {event.audio_action}</span></div></article>) : <div className="empty-event">Chưa có sự kiện — RoadWatch đang biết im lặng.</div>}</div></section>;
}
...
function EngineerConsole({ token, status }: { token: string; status: Status }) {
  ...
  return <main className="page engineer-page">
    ...
    <div className="metrics-row"><Metric label="Processed FPS" value={status.metrics.processed_fps} /><Metric label="Frame drop" value={`${(status.metrics.frame_drop_ratio * 100).toFixed(1)}%`} /><Metric label="E2E P50" value={`${latencies.end_to_end?.p50_ms ?? 0} ms`} /><Metric label="E2E P95" value={`${latencies.end_to_end?.p95_ms ?? 0} ms`} /><Metric label="Suppressed" value={status.metrics.alerts_suppressed} hint="Cảnh báo bị loại bởi confirmation/cooldown/audio budget." /></div>
