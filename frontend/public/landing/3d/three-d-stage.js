...
// SỬA SO VỚI BẢN GỐC: bare specifier 'three' đổi thành đường dẫn tương đối,
// vì trang chạy dưới CSP `script-src 'self'` nên không dùng được import map.
/* BEGIN USAGE */
/**
  ...
  class ThreeDStage extends HTMLElement {
    ...
    async _boot() {
      ...
      const [THREE, controlsMod] = await Promise.all([
        ...
        import('./vendor/three/addons/OrbitControls.js'),
      ]);
      this._THREE = THREE;
    ...
    async _exportObj() {
      if (!this._object) return;
      const mod = await import('./vendor/three/addons/OBJExporter.js');
    ...
    async _exportGlb() {
      if (!this._object) return;
      const mod = await import('./vendor/three/addons/GLTFExporter.js');
      this._nameParts();
      const base = this._basename;
