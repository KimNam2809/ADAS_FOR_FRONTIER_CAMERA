"""RoadWatch Object V3 full fine-tune on Kaggle GPU.

The candidate is deliberately isolated from the production model. It reuses
the validated dataset materialization path and exports a checkpoint/ONNX pair
for post-training Quality Gate evaluation before any promotion.
"""
...
DATASET = TEMP / "roadwatch_object_v3_full_dataset"
TARGET = TEMP / "roadwatch_target_videos"
FRAMES = TEMP / "roadwatch_target_frames"
OUTPUT = WORK / "roadwatch_object_v3_full"
RUN = OUTPUT / "roadwatch_objects_v3_pilot"
...
RUN_MODE = os.environ.get("ROADWATCH_RUN_MODE", "full")
if RUN_MODE not in {"pilot_1_epoch", "pilot_3_5_epoch", "full"}:
    raise RuntimeError(f"Unsupported ROADWATCH_RUN_MODE: {RUN_MODE}")
EPOCHS = {"pilot_1_epoch": 1, "pilot_3_5_epoch": 5, "full": 80}[RUN_MODE]
RUN_NAME = {
    "pilot_1_epoch": "roadwatch_objects_v3_pilot_1e",
    "pilot_3_5_epoch": "roadwatch_objects_v3_smoke_5e",
    "full": "roadwatch_objects_v3_full",
}[RUN_MODE]
    ...
    for path in paths:
        ...
        with path.open("rb") as stream:
            for chunk in iter(lambda: stream.read(1024 * 1024), b""):
                digest.update(chunk)
        result[str(path.relative_to(WORK))] = digest.hexdigest()
    return result
...
def write_data_yaml() -> Path:
    ...
    path = DATASET / "roadwatch-object-v3-pilot.yaml"
    path.write_text(
        yaml.safe_dump(
            {
                "path": str(DATASET),
...
def train(data_yaml: Path, checkpoint: Path, gpu_count: int) -> dict:
    ...
    model.train(
        ...
        seed=SEED,
        deterministic=True,
        patience=0 if RUN_MODE == "pilot_1_epoch" else (2 if RUN_MODE == "pilot_3_5_epoch" else 15),
        close_mosaic=0 if RUN_MODE == "pilot_1_epoch" else 5,
        lr0=0.001,
        lrf=0.01,
        save_period=1,
        project=str(OUTPUT),
...
def main() -> int:
    ...
    try:
        ...
        write_json(
            ...
            {
                "status": "PASS" if pseudo["training_allowed"] else "BLOCKED",
                "run_mode": RUN_MODE,
                "taxonomy": CANONICAL,
                "base": base,
                "video_profile": video_profile,
                "pseudo_labels": {key: value for key, value in pseudo.items() if key != "audit_samples"},
                "target_labels_are_pseudo": True,
                "promotion_allowed": False,
            },
        )
        ...
        status = {
            "status": "complete_pilot" if RUN_MODE.startswith("pilot") else "complete_candidate_not_promoted",
            "run_mode": RUN_MODE,
            "training_error": None,
            "versions": versions,
            "base": base,
    ...
    if status.get("training"):
        write_json(
            ...
            artifact_hashes([Path(status["training"]["best"]), Path(status["training"]["onnx"]), OUTPUT / "job_status.json"]),
        )
    print(json.dumps(status, ensure_ascii=False, indent=2), flush=True)
    return 0
