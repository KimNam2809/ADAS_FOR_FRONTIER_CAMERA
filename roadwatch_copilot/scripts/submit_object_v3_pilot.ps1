...
$kernelPath = Join-Path $projectRoot "kaggle\train_object_v3_pilot"
...
if (-not (Test-Path -LiteralPath (Join-Path $kernelPath "kernel-metadata.json"))) {
    throw "Thiếu kernel-metadata.json của Object V3 pilot."
}
...
Write-Host "Submitting RoadWatch Object V3 pilot (GPU, 1 epoch default)..."
& $kaggle kernels push -p $kernelPath
if ($LASTEXITCODE -ne 0) {
    throw "Kaggle push thất bại với exit code $LASTEXITCODE"
}
& $kaggle kernels status "lekimnam/roadwatch-object-detector-v3-pilot"
if ($LASTEXITCODE -ne 0) {
    throw "Không đọc được trạng thái Object V3 pilot với exit code $LASTEXITCODE"
}
