...
$kernelPath = Join-Path $projectRoot "kaggle\train_object_v3_smoke"
...
if (-not (Test-Path -LiteralPath (Join-Path $kernelPath "kernel-metadata.json"))) {
    throw "Thiếu kernel-metadata.json của Object V3 smoke."
}
...
Write-Host "Submitting RoadWatch Object V3 smoke (GPU, 5 epochs)..."
& $kaggle kernels push -p $kernelPath
if ($LASTEXITCODE -ne 0) {
    throw "Kaggle push thất bại với exit code $LASTEXITCODE"
}
& $kaggle kernels status "lekimnam/roadwatch-object-detector-v3-smoke-5-epochs"
if ($LASTEXITCODE -ne 0) {
    throw "Không đọc được trạng thái Object V3 smoke với exit code $LASTEXITCODE"
}
