...
Alert Governor (deterministic)
  ...
  └─ one audio winner; all accepted events remain on HUD
        ...
        └────────► FastAPI/WebSocket/MJPEG ► React HMI
                                      └──────► optional async SLM ► Engineer Console
```

SLM là nhánh giải thích hậu xử lý, chỉ nhận canonical event đã được
`AlertGovernor` chấp nhận. Nhánh này có queue giới hạn, không nằm trên critical
path và tự fallback về trạng thái deterministic khi model/validator/queue lỗi.
Chi tiết xem [SLM_INTEGRATION.md](SLM_INTEGRATION.md).
