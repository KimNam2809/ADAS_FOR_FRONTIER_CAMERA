...
Cloud Run chỉ tải bốn model active allowlist vào filesystem tạm khi khởi động.
Danh mục video được đọc từ metadata trong `cloud_assets.json`; video được tải
lười từ GCS khi người dùng thật sự chọn preview/phân tích. Cách này tránh tải
khoảng 252 MB video ở mọi cold start. Production vẫn cần signed GCS URL và replay
worker bất đồng bộ để không phụ thuộc filesystem của Cloud Run.

Cloud full profile dùng `yolo11n_320.onnx`, `roadwatch_detector_v2_416.onnx`,
`roadwatch_speed_digits_v2.onnx` và `yolop_lane_detection_640.onnx` trên CPU.
Local/AAOS giữ release profile riêng. Cloud container không phát loa server;
browser phát beep và WAV Piper từ private TTS service. Đây là cấu hình
demo/evaluation, không phải edge safety benchmark.

SLM Qwen2.5-0.5B là thành phần tùy chọn và hiện **không được bật** trong
cloudbuild mặc định. Nếu muốn chạy thử explanation cho Engineer Console, cần
upload ba asset Qwen vào `models/qwen2.5-0.5b/` trong bucket, thêm chúng vào
`ROADWATCH_ASSET_PATHS`, rồi triển khai revision riêng với
`ROADWATCH_SLM_ENABLED=1`. Không đưa SLM vào critical audio path; nếu revision
SLM lỗi, giữ `ROADWATCH_SLM_ENABLED=0` để Cloud Run tiếp tục dùng pipeline
deterministic và Piper như trước.

Để phù hợp giai đoạn chấm bài không biết trước thời điểm truy cập, Web dùng
8 vCPU/8 GiB, concurrency 80, `min=0,max=1`, session affinity và instance-based
CPU (`--no-cpu-throttling`). Khi người dùng mở URL, React shell hiển thị startup
gate trong khi GCS/model/TTS được khởi tạo nền; `/api/session/start` trả 503 cho
tới khi `/api/startup.ready=true`. Warm performance được giữ nguyên, còn cold
start được báo riêng và không tính vào E2E inference latency.
