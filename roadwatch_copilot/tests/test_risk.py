...
def test_sparse_cloud_sign_samples_keep_temporal_identity_and_emit_speed_event() -> None:
    engine = RiskEngine(CONFIG)
    frame = np.zeros((1080, 1920, 3), dtype=np.uint8)
    boxes = [
        [1125.08, 401.26, 1160.12, 439.91],
        [1148.27, 380.92, 1186.46, 423.75],
        [1259.05, 290.90, 1317.53, 355.20],
    ]
    candidates = []
    for timestamp, bbox in zip([3.333, 3.867, 4.667], boxes):
        candidates, _, _ = engine.analyze(
            ...
            [{
                ...
                "label": "60",
                "confidence": 0.97,
                "speed_classifier_confidence": 0.99,
            }],
            {
                "lane_mask": np.zeros((1080, 1920), dtype=np.uint8),
                "drivable_mask": np.zeros((1080, 1920), dtype=np.uint8),
                "quality": 0.0,
            },
            (1080, 1920),
            sign_fresh=True,
    ...
    speed_events = [item for item in candidates if item["event_type"] == "speed_sign"]
    assert len(speed_events) == 1
    assert speed_events[0]["evidence"]["speed_value"] == 60
    assert speed_events[0]["message"] == (
...
def test_candidate_rider_class_is_treated_as_vulnerable_road_user() -> None:
    ...
    assert any("Xe hai bánh" in candidate["message"] for candidate in candidates)
...
def test_emergency_bbox_override_prevents_silent_fcw_after_closing_rate_drops() -> None:
    ...
    assert fcw["message"] == "Cảnh báo va chạm"
...
def test_pedestrian_never_becomes_cut_in() -> None:
    engine = RiskEngine(CONFIG)
    bad_lane = {
        "lane_mask": np.zeros((480, 640), dtype=np.uint8),
        "drivable_mask": np.zeros((480, 640), dtype=np.uint8),
    ...
    pedestrian = {
        ...
        "age_seconds": 1.2,
        "expansion_rate": 0.03,
        "lateral_velocity": -0.12,
        "origin_x_norm": 0.86,
        "displacement_x_norm": -0.10,
        "displacement_y_norm": 0.02,
        "motion_observations": 8,
    }
    candidates, _, _ = engine.analyze(
        [pedestrian], [], bad_lane, (480, 640), sign_fresh=False
    )
    assert not any(item["event_type"] == "cut_in" for item in candidates)


def test_safe_pedestrian_crossing_uses_cross_traffic_copy() -> None:
    engine = RiskEngine(CONFIG)
    bad_lane = {
        "lane_mask": np.zeros((480, 640), dtype=np.uint8),
        "drivable_mask": np.zeros((480, 640), dtype=np.uint8),
    ...
    crossing = next(item for item in candidates if item["event_type"] == "cross_traffic")
    assert crossing["message"] == "Người đi bộ đang cắt ngang từ trái sang phải."
    assert crossing["evidence"]["crossing_profile"] == "pedestrian_safe_crossing"
