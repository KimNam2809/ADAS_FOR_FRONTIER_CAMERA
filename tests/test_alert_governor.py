...
def test_overlong_external_message_uses_safe_short_fallback() -> None:
    governor = AlertGovernor(config())
    item = candidate()
    item["message"] = "Cảnh báo rất dài không được phát cho tài xế trong một câu duy nhất"

    events, _ = governor.decide([item], now=100.0)

    assert events[0]["message"] == "Hãy chú ý."
    assert events[0]["display_message"] == events[0]["spoken_message"] == "Hãy chú ý."
    assert events[0]["evidence"]["message_budget_fallback"] is True
    assert events[0]["evidence"]["original_message_word_count"] > 7
...
def test_sign_audio_retries_after_global_gap_instead_of_waiting_full_cooldown() -> None:
    ...
    sign = {
        **candidate("advisory", "speed_sign:60", 0.3),
        ...
        "message": "Tối đa 60 ki-lô-mét/giờ.",
    }
...
def test_first_sign_near_video_clock_zero_receives_tts() -> None:
    ...
    sign = {
        **candidate("advisory", "speed_sign:50", 0.3),
        ...
        "object_id": None,
        "is_traffic_sign": True,
        "message": "Tối đa 50 ki-lô-mét/giờ.",
    }
...
def test_confirmed_speed_sign_is_spoken_after_gap_even_when_sign_left_frame() -> None:
    ...
    sign = {
        **candidate("advisory", "speed_sign:80", 0.3),
        ...
        "object_id": None,
        "is_traffic_sign": True,
        "message": "Tối đa 80 ki-lô-mét/giờ.",
    }
